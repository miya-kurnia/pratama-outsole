                        ...
                        <tbody>
                            @foreach ($orders as $index => $order)
                                <tr class="border-b dark:border-gray-700">
                                    ...
                                    <td class="px-4 py-3">{{ $order->po }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->model_name }}
                                    </td>
                                    <td class="px-4 py-3 max-w-[12rem] truncate">{{ $order->style }}</td>
                                    <td class="px-4 py-3">{{ $order->gender }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->total_size }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->size_36 }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->size_37 }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->size_38 }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->size_39 }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->size_40 }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->size_41 }}
                                    </td>
                                    <td class="px-4 py-3">{{ $order->size_42 }}
                                    </td>
                                    <!-- Action button with dropdown -->
                                    <td class="flex items-center justify-end px-4 py-3">
                                        <button id="dropdown-button-{{ $order->id }}"
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <form action="{{ route('orders.store') }}" method="POST">
                    ...
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        <div>
                            <label for="po"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">PO</label>
                        ...
                        <div>
                            ...
                            <input type="text" name="style" id="style"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Enter style name" required="">
                        </div>
                        <div>
                            <label for="gender"
                ...
                <form action="{{ route('orders.update', $order->id) }}" method="POST">
                    ...
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        <div>
                            ...
                            <input type="text" name="po" id="po"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                value="{{ old('po', $order->po) }}" placeholder="Enter PO number" required>
                        </div>
                        <div>
                            <label for="model_name"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Model Name</label>
                            <input type="text" name="model_name" id="model_name"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                value="{{ old('model_name', $order->model_name) }}" placeholder="Enter model name"
                                required>
                        </div>
                        <div>
                            <label for="style"
                            ...
                            <input type="text" name="style" id="style"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                value="{{ old('style', $order->style) }}" placeholder="Enter style name" required>
                        </div>
                        <div>
                            <label for="gender"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Gender</label>
                            <input type="text" name="gender" id="gender"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                value="{{ old('gender', $order->gender) }}" placeholder="Enter gender" required>
                        </div>
                        @foreach (range(36, 42) as $size)
                            <div>
                                <label for="size_{{ $size }}"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Size Run
                                    {{ $size }}</label>
                                <input type="number" name="size_{{ $size }}" id="size_{{ $size }}"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    value="{{ old('size_' . $size, $order->{'size_' . $size}) }}"
                                    placeholder="Enter size range">
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <div class="flex items-center justify-center space-x-4">
                    ...
                    <form action="{{ route('orders.destroy', $order->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        {{-- <button class="px-2 py-2 text-sm text-red-600" type="submit">{{ __('Delete') }}</button> --}}
                        <button type="submit"
